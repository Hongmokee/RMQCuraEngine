    ...
    void writeGCode(SliceDataStorage& storage)
    {
        ...
        if (config.raftBaseThickness > 0 && config.raftInterfaceThickness > 0)
        {
...

            {
                ...
                gcode.writeLine("M117 RAFT (%d/%d)...", -2, totalLayers);
...

            {
                ...
                gcode.writeComment("RAFT");
                gcode.writeLine("M117 RAFT (%d/%d)...", -1, totalLayers);
                GCodePlanner gcodeLayer(gcode, config.moveSpeed, config.retractionMinimalDistance);
...
#undef SPEED_SMOOTH
                ...
                skinConfig.setData(config.skinSpeed, extrusionWidth, "FILL"); //"SKIN"
                supportConfig.setData(config.printSpeed, extrusionWidth, "SUPPORT");
            }

            gcode.writeComment("LAYER:%d", layerNr);
            float progress = (float)layerNr/(float)totalLayers*100.0;
            gcode.writeLine("M117 *%d%% (%d/%d)...", (int)progress, layerNr, totalLayers);
            ...
            if (printSupportFirst)
            {
                ...
                setFanSpeed(gcodeLayer, layerNr);
            ...
            {
            	GCodePlanner gcodeLayer_Volume(gcode, config.moveSpeed, config.retractionMinimalDistance);

                if (volumeCnt > 0)
                    volumeIdx = (volumeIdx + 1) % storage.volumes.size();
                addVolumeLayerToGCode(storage, gcodeLayer_Volume, volumeIdx, layerNr);
                setFanSpeed(gcodeLayer_Volume, layerNr);
                gcodeLayer_Volume.writeGCode(config.coolHeadLift > 0, static_cast<int>(layerNr) > 0 ? config.layerThickness : config.initialLayerThickness);

            }
            if (!printSupportFirst)
            {
            	GCodePlanner gcodeLayer_Support(gcode, config.moveSpeed, config.retractionMinimalDistance);
                addSupportToGCode(storage, gcodeLayer_Support, layerNr);
                setFanSpeed(gcodeLayer_Support, layerNr);
                gcodeLayer_Support.writeGCode(config.coolHeadLift > 0, static_cast<int>(layerNr) > 0 ? config.layerThickness : config.initialLayerThickness);
            }
    ...
    {
    	changeExtrution(layerNr, config.filamentFlow, 70); //Extruder Factor Change
    	gcodeLayer.setExtrudeSpeedFactor(100);
        ...
        for(unsigned int n=0; n<supportIslands.size(); n++)
        {
            ...
            if (config.supportLineDistance > 0)
            {
                ...
                case SUPPORT_TYPE_GRID:
                    ...
                    {
                        generateLineInfill(island, supportLines, config.extrusionWidth, config.supportLineDistance*2, config.SupportOverlap, 0);
                        generateLineInfill(island, supportLines, config.extrusionWidth, config.supportLineDistance*2, config.SupportOverlap, 90);
                    }else{
                        generateLineInfill(island, supportLines, config.extrusionWidth, config.supportLineDistance, config.SupportOverlap, (layerNr & 1) ? 0 : 90);
                    }
                    break;
                case SUPPORT_TYPE_LINES:
                    if (layerNr == 0)
                    {
                        generateLineInfill(island, supportLines, config.extrusionWidth, config.supportLineDistance, config.SupportOverlap + 150, 0);
                        generateLineInfill(island, supportLines, config.extrusionWidth, config.supportLineDistance, config.SupportOverlap + 150, 90);
                    }else{
                        generateLineInfill(island, supportLines, config.extrusionWidth, config.supportLineDistance, config.SupportOverlap, 0);
                    }
                    break;
                }
            }
    ...
    void changeExtrution(int layerNr, int filamentFlow, int factor) //factor 0~100
    {
    	if (layerNr == 0)
    	    gcode.setExtrusion(config.initialLayerThickness, config.filamentDiameter, filamentFlow);
		else
			gcode.setExtrusion(config.layerThickness, config.filamentDiameter, filamentFlow*factor/100);
    }

    void setFanSpeed(GCodePlanner& gcodeLayer, int layerNr)
    {
