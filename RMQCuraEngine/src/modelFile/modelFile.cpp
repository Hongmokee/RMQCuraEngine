...
SimpleModel* loadModelOBJ(SimpleModel *m,const char* filename, FMatrix3x3& matrix)
{
	int flen,vlen;
	int a[3];
	int preFace[3];
    ...
    while(fgets_(buffer, sizeof(buffer), f))
    {
		...
		{
			sscanf(buffer, "v %lf %lf %lf", &vertex.x, &vertex.y, &vertex.z);
			verlist.vertexs.push_back(vertex);
		}
		else if(buffer[0]=='f')
		{
			tmp = strtok(buffer, " ");
			...
			face.x = a[0]; face.y = a[1]; face.z = a[2];
			faclist.faces.push_back(face);
		}
	}
	fclose(f);

	vlen=verlist.vertexs.size();
	flen=faclist.faces.size();

	for(int i = 0; i<flen; i++)
	{
		preFace[0]=faclist.faces[i].x-1;
		preFace[1]=faclist.faces[i].y-1;
		preFace[2]=faclist.faces[i].z-1;

		for(int j = 0; j<3; j++)
		{
			if ((preFace[j] < 0)||(preFace[j] >= vlen))
			{
				preFace[j] = 0;
			}
		}

		v[0]=verlist.vertexs[preFace[0]].x;
		v[1]=verlist.vertexs[preFace[0]].y;
		v[2]=verlist.vertexs[preFace[0]].z;
		v[3]=verlist.vertexs[preFace[1]].x;
		v[4]=verlist.vertexs[preFace[1]].y;
		v[5]=verlist.vertexs[preFace[1]].z;
		v[6]=verlist.vertexs[preFace[2]].x;
		v[7]=verlist.vertexs[preFace[2]].y;
		v[8]=verlist.vertexs[preFace[2]].z;
...
SimpleModel* loadModelFromFile(SimpleModel *m,const char* filename, FMatrix3x3& matrix)
{
    ...
    else if (ext && stringcasecompare(ext, ".obj") == 0)
    {
        return loadModelOBJ(m,filename, matrix);
    }
