...
int main(int argc, char **argv)
{
    ...
    processor.guiConnect(15000);
    int commandLineCount=setArr(processor.buffer);
    //cura::logError("this is %s\n", processor.buffer);
    ...
    cura::logError("Modified by RedMosQ\n");
    cura::logError("\n");
    ...
    for(int argn = 1; argn < commandLineCount; argn++)
        cura::log("Arg: %s\n", arr[argn]);
        ...
        if (str[0] == '-')
        {
            for(str++; *str; str++)
            {
                ...
                case 'b':
                    ...
                    binaryMeshBlob = fopen(arr[argn], "rb");
                    break;
                case 'o':
                    argn++;
                    if (!processor.setTargetFile(arr[argn]))
                    {
                ...
                case 'c':
                    {
                        ...
                        argn++;
                        if(!config.readSettings(arr[argn])) {
                ...
                case 's':
                    {
                        ...
                        argn++;
                        char* valuePtr = strchr(arr[argn], '=');
                        if (valuePtr)
                        {
                            *valuePtr++ = '\0';

                            if (!config.setSetting(arr[argn], valuePtr))
                ...
                case 'm':
                    ...
                    argn++;
                    sscanf(arr[argn], "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf",
